...
const EditSpotlightCelebrityModal = ({
  isOpen,
  onClose,
  onSubmit,
  loading,
  ...
  useEffect(() => {
    if (celebrityData && isOpen) {
      setImagePreview(celebrityData.image?.url || null)
      setName(celebrityData.name || '')
      setType(celebrityData.type || '')
      setAltText(celebrityData.image?.altText || '')
      setImage(null)
      setErrors({})
    }
  }, [celebrityData, isOpen])
  ...
  const handleSubmit = () => {
    ...
    onSubmit(celebrityData._id, formData)
  }
...
export default EditSpotlightCelebrityModal
